...
void place() {
  ...
  while(netTime < 5000) { //Sets timeout to 3 seconds
    ...
    if(TrayM.get_position() > -3000) { //Sets boundaries for fast movement using encoder position
      TrayM = -127;
    } else if(TrayM.get_position() <= -3000 && TrayM.get_position() > -4000) { //Changes speeds based on encoder positions
      TrayM = -80;
    } else if (TrayM.get_position() <= -4000 && TrayM.get_position() > -5500) {
      TrayM = -60;
    }
